...
unsigned int CAN_msg_Buf[32][8] __attribute__((space(dma),aligned(512))); 
...
//volatile unsigned int * DMAiSTA[] = {&DMA0STA, &DMA1STA, &DMA2STA, &DMA3STA, &DMA4STA, &DMA5STA, &DMA6STA, &DMA7STA};
...
{
	char temp_win;
	unsigned int ii, Buf_read_ptr, filter_hit;
	unsigned long ID_rx;
